...
export const EMRProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const [role, setRoleState] = useState<UserRole | null>(() => {
    const saved = sessionStorage.getItem('emr_role');
  ...
  const [currentGuardian, setCurrentGuardianState] = useState<GuardianUser | null>(() => {
    const saved = sessionStorage.getItem('emr_guardian');
    return saved ? JSON.parse(saved) : null;
  });
  const [currentStaff, setCurrentStaffState] = useState<StaffMember | null>(() => {
    const saved = sessionStorage.getItem('emr_staff');
    return saved ? JSON.parse(saved) : null;
  });

  const setRole = (r: UserRole | null) => {
    setRoleState(r);
    if (r) sessionStorage.setItem('emr_role', r);
    else sessionStorage.removeItem('emr_role');
  };

  const setCurrentUser = (u: Patient | null) => {
    setCurrentUserState(u);
    if (u) sessionStorage.setItem('emr_user', JSON.stringify(u));
    else sessionStorage.removeItem('emr_user');
  };

  const setCurrentGuardian = (g: GuardianUser | null) => {
    setCurrentGuardianState(g);
    if (g) sessionStorage.setItem('emr_guardian', JSON.stringify(g));
    else sessionStorage.removeItem('emr_guardian');
  };

  const setCurrentStaff = (s: StaffMember | null) => {
    setCurrentStaffState(s);
    if (s) sessionStorage.setItem('emr_staff', JSON.stringify(s));
    else sessionStorage.removeItem('emr_staff');
  };
