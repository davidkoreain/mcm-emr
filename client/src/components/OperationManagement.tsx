      ...
      {addItemModal && (
        <div style={overlayStyle}>
          <div style={boxStyle}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
              {[
                { label: 'Item Name *', key: 'item', placeholder: 'e.g. Surgical Gloves' },
                { label: 'Quantity', key: 'qty', placeholder: '1', type: 'number' },
                { label: 'Unit', key: 'unit', placeholder: 'e.g. Pair, Vial, Pack' },
              ].map(({ label, key, placeholder, type }) => (
                <div key={key}>
                  <label style={{ fontSize: '0.875rem', fontWeight: '600', display: 'block', marginBottom: '0.35rem' }}>{label}</label>
                  <input type={type || 'text'} value={(newItem as any)[key]} placeholder={placeholder}
                    ...
                    <tr key={sc.id}>
                      ...
                      <td>
                        <span className={`status-badge`} style={{
                          background: sc.status === 'Ongoing' ? '#fef3c7' : sc.status === 'Completed' ? '#dcfce7' : sc.status === 'Cancelled' ? '#fee2e2' : '#f1f5f9',
                          color: sc.status === 'Ongoing' ? '#92400e' : sc.status === 'Completed' ? '#166534' : sc.status === 'Cancelled' ? '#991b1b' : '#64748b',
                        }}>{sc.status}</span>
                      </td>
                      <td>
                        <button className="btn-secondary" style={{ padding: '0.4rem 0.8rem', fontSize: '0.8rem' }} onClick={() => setManageModal(sc)}>Manage</button>
                      </td>
                    </tr>
                  ))}
                </tbody>
            ...
            <div className="stat-card" style={{ padding: '1.5rem' }}>
              ...
              {currentResources.staff.map((s, idx) => (
                <div key={idx} style={{ display: 'flex', justifyContent: 'space-between', padding: '1rem', borderBottom: '1px solid #eee' }}>
                  <div><div style={{ fontWeight: '600' }}>{s.name}</div><div style={{ fontSize: '0.8rem', color: 'var(--text-secondary)' }}>{s.dept}</div></div>
                  <span className="status-badge" style={{ background: '#f1f5f9', color: '#475569' }}>{s.role}</span>
                </div>
              ))}
              <button className="btn-secondary" style={{ width: '100%', marginTop: '1rem' }} onClick={() => alert('Staff assignment: connect to HR module to assign staff.')}>Assign More Staff</button>
            </div>
            <div className="stat-card" style={{ padding: '1.5rem' }}>
              <h3 style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', marginBottom: '1.25rem' }}>
              ...
              {currentResources.equipment.map((e, idx) => (
                <div key={idx} style={{ display: 'flex', justifyContent: 'space-between', padding: '1rem', borderBottom: '1px solid #eee' }}>
                  <div><div style={{ fontWeight: '600' }}>{e.name}</div><div style={{ fontSize: '0.8rem', color: 'var(--text-secondary)' }}>ID: {e.id}</div></div>
                  <span className={`status-badge ${e.status.includes('Available') ? 'status-active' : 'status-pending'}`}>{e.status}</span>
                </div>
              ))}
              <button className="btn-secondary" style={{ width: '100%', marginTop: '1rem' }} onClick={() => alert('Equipment request sent to Asset Management DB.')}>Request Equipment (Asset DB)</button>
            </div>
          </div>
        )}
