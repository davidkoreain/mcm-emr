...
const ClinicalEncounter: React.FC<ClinicalEncounterProps> = ({ onClose, patientName }) => {
  ...
  const imagingData = [
    ...
    { id: 3, type: 'Video', title: 'Endoscopy Procedure', date: '2026-05-05', thumb: 'https://images.unsplash.com/photo-1579154235602-44373db99a23?auto=format&fit=crop&q=80&w=200', isVideo: true },
  ];
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    console.log('SOAP Note Submitted:', soap);
    alert('Clinical Encounter saved successfully!');
    onClose();
  };
                  ...
                  <div key={hist.id} style={{ position: 'relative', marginBottom: '2rem' }}>
                     ...
                     <div className="stat-card" style={{ padding: '1.5rem', height: 'auto', border: '1px solid #e2e8f0' }}>
                        ...
                        <div style={{ background: '#f1f5f9', padding: '0.75rem', borderRadius: '0.5rem', fontSize: '0.9rem', display: 'flex', alignItems: 'start', gap: '0.5rem' }}>
                           <Pill size={16} style={{ marginTop: '2px' }} />
                           <div>{hist.meds}</div>
                        </div>
...
const Pill: React.FC<{ size?: number, style?: React.CSSProperties }> = ({ size = 24, style }) => (
  <svg 
    width={size} 
    height={size} 
    viewBox="0 0 24 24" 
